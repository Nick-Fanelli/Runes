...
public class TileMap {
    ...
    public TileMap(State state, LDtkLevel level) {
        ...
        this.texture = AssetManager.LoadTexture(state, "tileset.png");
        this.spriteSheet = new SpriteSheet(texture, 8, 8);
    }
    ...
    public void GenerateGameObjects() {
        for(LDtkLayer layer : level.ldtkLayers) {
            ...
            for(LDtkTile tile : layer.ldtkTiles) {
                ...
                gameObject.AddComponent(new SpriteRendererComponent(new Vector4f(1.0f), this.texture, this.spriteSheet.GetSprite(tile.t)));
